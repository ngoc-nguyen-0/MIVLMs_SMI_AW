...
def inversion_attack(args,model, tokenizer,  G, ws, input_ids,label_ids, attack_transform, file_path,device,hidden_states_mean, hidden_states_var,n_epochs=100, lr=0.05):
    ...
    for i in range(int(ws.shape[0]/batchsize)):
        ...
        if os.path.exists(w_filepath) == False:                 
            ...
            for epoch in range(n_epochs):
                ...
                with autocast():
                    ...
                    if args.loss =="LOM":                        
                        ...
                        loss = criterion(label, output_, hidden_states, hidden_states_mean, hidden_states_var) 
                    else:                  
                        loss = criterion(output_,label)
                    ...
                    print(f"Epoch {epoch}: Loss = {loss.item()}, {generated_text}, {correct_class_probs_.tolist()}")
            ...
            torch.save({'w':w.detach(), 'all_loss':all_loss, 'output_text': output_text, 'all_w':all_w, 'correct_class_probs':correct_class_probs.numpy()}, w_filepath)
