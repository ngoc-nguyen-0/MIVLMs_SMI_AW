...
def inversion_attack(args,model, tokenizer,  G, ws, input_ids,label_ids, attack_transform, file_path,device,hidden_states_mean, hidden_states_var,n_epochs=100, lr=0.05):
    ...
    elif args.loss =="MML_batch":
         criterion = max_margin_loss_batch
    ...
    for i in range(int(ws.shape[0]/batchsize)):
        ...
        if os.path.exists(w_filepath) == False:                 
            ...
            index_visual_tokens = torch.where(input_ids[0]==-200)[0]
            ...
            for epoch in range(n_epochs):
                ...
                with autocast():
                    ...
                    attentions = []
                    for ii in range(-index,-2, 1):
                        image_tokens_attn = np.array([output['attentions'][layer][0, :, ii, index_visual_tokens:index_visual_tokens+NUM_IMG_TOKENS].mean(dim=0).to(torch.float32).detach().cpu().numpy() for layer in range(32)])
                        image_tokens_attn=np.sum(np.sum(image_tokens_attn,axis=1))
                        
                        attentions.append(image_tokens_attn)

                    token_w = np.array(attentions)/sum(attentions)
                    token_w = torch.from_numpy(token_w)


                    logits = output.logits[0]
                    output_ = logits[-index:-2] ## remove ending token
                    ...
                    with torch.no_grad():                        
                        ...
                        token_w =token_w.to(device)
                        weight_token.append(token_w)
                    if args.loss =="LOM":                        
                        hidden_states = output.hidden_states[0][-index:-2]
                        loss = criterion(label, output_, hidden_states, hidden_states_mean, hidden_states_var, w= token_w) 
                    else:                  
                        loss = sum(criterion(output_,label)*token_w)
                    ...
                    print(f"Epoch {epoch}: Loss = {loss.item()}, {generated_text}, {correct_class_probs_.tolist()}, {token_w}")
            ...
            weight_token = torch.stack(weight_token).detach().cpu().numpy()
            torch.save({'w':w.detach(), 'all_loss':all_loss, 'output_text': output_text, 'all_w':all_w, 'correct_class_probs':correct_class_probs.numpy(), 'weight_token': weight_token}, w_filepath)
